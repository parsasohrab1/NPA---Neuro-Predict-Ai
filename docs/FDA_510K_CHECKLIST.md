# چک‌لیست کامل آماده‌سازی FDA 510(k) Submission
# Complete FDA 510(k) Submission Checklist

> ## Evidence status (honesty banner)
>
> **Current execution / evidence completion: 0%.**  
> This checklist is a planning scaffold. No FDA forms have been filed, no
> predicate analysis package is attached, and no clinical validation artifacts
> are checked in under `forms/` (directory absent). A draft device description
> exists at `docs/FDA_510K_Device_Description.md` — that is **documentation
> scaffolding**, not submission evidence. Do **not** raise completion
> percentages without citing real artifacts (see `docs/EVIDENCE_PACK_INDEX_FA.md`).

## 📋 وضعیت کلی

| بخش | وضعیت | درصد تکمیل | مسئول | تاریخ هدف |
|-----|-------|-------------|-------|-----------|
| فرم‌ها و اطلاعات اولیه | ⬜ | 0% | Regulatory Affairs | |
| اطلاعات دستگاه | ⬜ | 0% | Engineering | |
| Substantial Equivalence | ⬜ | 0% | Regulatory Affairs | |
| شواهد بالینی | ⬜ | 0% | Clinical Affairs | |
| برچسب‌گذاری | ⬜ | 0% | Regulatory Affairs | |
| سیستم کیفیت | ⬜ | 0% | Quality Assurance | |
| اطلاعات شرکت | ⬜ | 0% | Regulatory Affairs | |
| **جمع کل** | ⬜ | **0%** | | |

**آخرین بازبینی صداقت:** ژوئیه ۲۰۲۶ — بدون تغییر صعودی درصد بدون شاهد.

---

## ✅ بخش 1: فرم‌ها و اطلاعات اولیه

### 1.1 فرم‌های اصلی FDA

- [ ] **فرم FDA 3514** (510(k) Submission Form)
  - [ ] Section A: Applicant Information
  - [ ] Section B: Device Information
  - [ ] Section C: Predicate Device Information
  - [ ] Section D: Indications for Use
  - [ ] Section E: Classification Information
  - [ ] Section F: Contact Information
  - [ ] Section G: Certification
  - **فایل**: `forms/FDA_3514_510k_Form.pdf`
  - **وضعیت**: ⬜ Not Started

- [ ] **فرم FDA 3674** (Truthful and Accurate Statement)
  - [ ] Signed by authorized representative
  - [ ] Date and signature
  - **فایل**: `forms/FDA_3674_Statement.pdf`
  - **وضعیت**: ⬜ Not Started

- [ ] **فرم FDA 3601** (User Fee Cover Sheet)
  - [ ] Fee amount: $22,745 (FY 2025)
  - [ ] Payment information
  - **فایل**: `forms/FDA_3601_Fee_Cover_Sheet.pdf`
  - **وضعیت**: ⬜ Not Started

### 1.2 نامه‌ها و خلاصه‌ها

- [ ] **Cover Letter**
  - [ ] Company letterhead
  - [ ] Date and submission number
  - [ ] Table of contents reference
  - [ ] Contact information
  - **فایل**: `FDA_510K_Cover_Letter.docx`
  - **وضعیت**: ⬜ Not Started

- [ ] **Executive Summary**
  - [ ] Device description (1 page)
  - [ ] Intended use
  - [ ] Predicate device identification
  - [ ] Summary of substantial equivalence
  - **فایل**: `docs/FDA_510K_Executive_Summary.md`
  - **وضعیت**: ⬜ Not Started

- [ ] **Table of Contents**
  - [ ] Complete list of all documents
  - [ ] Page numbers
  - [ ] Section references
  - **فایل**: `FDA_510K_Table_of_Contents.md`
  - **وضعیت**: ⬜ Not Started

- [ ] **Indications for Use Statement**
  - [ ] Clear statement of intended use
  - [ ] Patient population
  - [ ] Clinical setting
  - **فایل**: `docs/FDA_510K_Indications_for_Use.md`
  - **وضعیت**: ⬜ Not Started

---

## ✅ بخش 2: اطلاعات دستگاه

### 2.1 توصیف دستگاه

- [ ] **Device Description**
  - [ ] Complete device description
  - [ ] Components and architecture
  - [ ] Operating principles
  - [ ] Technical specifications
  - **فایل**: `docs/FDA_510K_Device_Description.md` ✅ Created
  - **وضعیت**: ✅ Created / ⬜ Review / ⬜ Approved

- [ ] **Device Specifications**
  - [ ] Functional specifications
  - [ ] Performance specifications
  - [ ] Input/output specifications
  - [ ] System requirements
  - **فایل**: `docs/FDA_510K_Device_Specifications.md`
  - **وضعیت**: ⬜ Not Started

- [ ] **Technical Performance Data**
  - [ ] Accuracy metrics
  - [ ] Precision and recall
  - [ ] Sensitivity and specificity
  - [ ] ROC curves
  - [ ] Confusion matrices
  - **فایل**: `docs/FDA_510K_Technical_Performance.md`
  - **وضعیت**: ⬜ Not Started

### 2.2 مستندات نرم‌افزار

- [ ] **Software Development Plan (SDP)**
  - [ ] Software classification (IEC 62304)
  - [ ] Development lifecycle model
  - [ ] Tools and environments
  - **فایل**: `docs/FDA_510K_Software_Development_Plan.md`
  - **وضعیت**: ⬜ Not Started

- [ ] **Software Requirements Specification (SRS)**
  - [ ] Functional requirements
  - [ ] Performance requirements
  - [ ] Interface requirements
  - **فایل**: `docs/SRS.md` ✅ Exists
  - **وضعیت**: ✅ Exists / ⬜ Review / ⬜ Approved

- [ ] **Software Architecture**
  - [ ] System architecture diagram
  - [ ] Component architecture
  - [ ] Data flow diagrams
  - **فایل**: `docs/FDA_510K_Software_Architecture.md`
  - **وضعیت**: ⬜ Not Started

- [ ] **Software Design Specification (SDS)**
  - [ ] Detailed design documents
  - [ ] Class diagrams
  - [ ] Sequence diagrams
  - **فایل**: `docs/FDA_510K_Software_Design.md`
  - **وضعیت**: ⬜ Not Started

- [ ] **Software Verification and Validation**
  - [ ] Unit testing results
  - [ ] Integration testing results
  - [ ] System testing results
  - [ ] Code coverage reports
  - **فایل**: `docs/FDA_510K_Software_VnV.md`
  - **وضعیت**: ⬜ Not Started

- [ ] **Software Risk Management**
  - [ ] Software hazard analysis
  - [ ] Risk assessment
  - [ ] Risk mitigation
  - **فایل**: `docs/FDA_510K_Software_Risk_Management.md`
  - **وضعیت**: ⬜ Not Started

---

## ✅ بخش 3: Substantial Equivalence

### 3.1 مقایسه با Predicate Device

- [ ] **Predicate Device Selection**
  - [ ] Identified predicate device
  - [ ] 510(k) number obtained
  - [ ] Device name and manufacturer
  - **فایل**: `docs/FDA_510K_Predicate_Device_Selection.md`
  - **وضعیت**: ⬜ Not Started

- [ ] **Predicate Device Comparison**
  - [ ] Side-by-side comparison table
  - [ ] Similarities documented
  - [ ] Differences documented
  - [ ] Justifications for differences
  - **فایل**: `docs/FDA_510K_Predicate_Comparison.md`
  - **وضعیت**: ⬜ Not Started

- [ ] **Comparative Testing**
  - [ ] Test methods documented
  - [ ] Test results obtained
  - [ ] Statistical comparison
  - [ ] Conclusions drawn
  - **فایل**: `docs/FDA_510K_Comparative_Testing.md`
  - **وضعیت**: ⬜ Not Started

### 3.2 Substantial Equivalence Statement

- [ ] **Substantial Equivalence Rationale**
  - [ ] Similar intended use
  - [ ] Similar technology
  - [ ] Similar safety and effectiveness
  - **فایل**: `docs/FDA_510K_SE_Rationale.md`
  - **وضعیت**: ⬜ Not Started

---

## ✅ بخش 4: شواهد بالینی

### 4.1 مطالعات بالینی (در صورت نیاز)

- [ ] **Clinical Study Protocol**
  - [ ] Study objectives
  - [ ] Study design
  - [ ] Inclusion/exclusion criteria
  - [ ] Sample size justification
  - [ ] Statistical analysis plan
  - **فایل**: `docs/FDA_510K_Clinical_Protocol.md`
  - **وضعیت**: ⬜ Not Started

- [ ] **Clinical Study Report**
  - [ ] Executive summary
  - [ ] Study conduct
  - [ ] Patient demographics
  - [ ] Results
  - [ ] Statistical analysis
  - [ ] Adverse events
  - **فایل**: `docs/FDA_510K_Clinical_Report.md`
  - **وضعیت**: ⬜ Not Started

- [ ] **Statistical Analysis Report**
  - [ ] Methods used
  - [ ] Results tables
  - [ ] Statistical significance
  - [ ] Confidence intervals
  - **فایل**: `docs/FDA_510K_Statistical_Analysis.md`
  - **وضعیت**: ⬜ Not Started

### 4.2 بررسی مقالات

- [ ] **Literature Review**
  - [ ] Relevant published studies
  - [ ] Similar devices
  - [ ] Clinical evidence summary
  - [ ] References (APA format)
  - **فایل**: `docs/FDA_510K_Literature_Review.md`
  - **وضعیت**: ⬜ Not Started

### 4.3 داده‌های بالینی

- [ ] **Clinical Data Summary**
  - [ ] Patient population
  - [ ] Clinical outcomes
  - [ ] Safety data
  - **فایل**: `docs/FDA_510K_Clinical_Data_Summary.md`
  - **وضعیت**: ⬜ Not Started

---

## ✅ بخش 5: برچسب‌گذاری

### 5.1 Proposed Labeling

- [ ] **Device Labeling**
  - [ ] Device name
  - [ ] Intended use statement
  - [ ] Indications for use
  - [ ] Contraindications
  - [ ] Warnings and precautions
  - [ ] Adverse events
  - **فایل**: `docs/FDA_510K_Proposed_Labeling.md`
  - **وضعیت**: ⬜ Not Started

- [ ] **Instructions for Use (IFU)**
  - [ ] Device description
  - [ ] Indications
  - [ ] Contraindications
  - [ ] Warnings
  - [ ] Precautions
  - [ ] Step-by-step instructions
  - [ ] Troubleshooting
  - **فایل**: `docs/FDA_510K_IFU.md`
  - **وضعیت**: ⬜ Not Started

- [ ] **User Manual**
  - [ ] Installation guide
  - [ ] User guide
  - [ ] Admin guide
  - [ ] Troubleshooting guide
  - [ ] FAQ
  - **فایل**: `docs/FDA_510K_User_Manual.md`
  - **وضعیت**: ⬜ Not Started

---

## ✅ بخش 6: سیستم کیفیت

### 6.1 Quality System Summary

- [ ] **Quality System Regulation Summary**
  - [ ] Quality policy
  - [ ] Organization structure
  - [ ] Quality procedures
  - [ ] Document control
  - [ ] Change control
  - **فایل**: `docs/FDA_510K_Quality_System_Summary.md`
  - **وضعیت**: ⬜ Not Started

### 6.2 Design Controls

- [ ] **Design Controls Documentation**
  - [ ] Design and development planning
  - [ ] Design input
  - [ ] Design output
  - [ ] Design review
  - [ ] Design verification
  - [ ] Design validation
  - [ ] Design transfer
  - **فایل**: `docs/FDA_510K_Design_Controls.md`
  - **وضعیت**: ⬜ Not Started

### 6.3 Risk Management

- [ ] **Risk Management Plan**
  - [ ] Scope and objectives
  - [ ] Risk management activities
  - [ ] Risk acceptance criteria
  - **فایل**: `docs/FDA_510K_Risk_Management_Plan.md`
  - **وضعیت**: ⬜ Not Started

- [ ] **Risk Analysis**
  - [ ] Hazard identification
  - [ ] Risk estimation
  - [ ] Risk evaluation
  - [ ] Hazard/Harm matrix
  - **فایل**: `docs/FDA_510K_Risk_Analysis.md`
  - **وضعیت**: ⬜ Not Started

- [ ] **Risk Control Measures**
  - [ ] Inherent safety by design
  - [ ] Protective measures
  - [ ] Information for safety
  - **فایل**: `docs/FDA_510K_Risk_Control.md`
  - **وضعیت**: ⬜ Not Started

- [ ] **Residual Risk Evaluation**
  - [ ] Evaluation of residual risk
  - [ ] Benefit-risk analysis
  - [ ] Risk acceptance rationale
  - **فایل**: `docs/FDA_510K_Residual_Risk.md`
  - **وضعیت**: ⬜ Not Started

### 6.4 Cybersecurity

- [ ] **Cybersecurity Documentation**
  - [ ] Security controls
  - [ ] Vulnerability assessment
  - [ ] Security testing
  - [ ] Incident response plan
  - **فایل**: `docs/FDA_510K_Cybersecurity.md`
  - **وضعیت**: ⬜ Not Started

---

## ✅ بخش 7: اطلاعات شرکت

### 7.1 Establishment Registration

- [ ] **FDA Establishment Registration**
  - [ ] Facility registered with FDA
  - [ ] Registration number obtained
  - [ ] Annual registration updated
  - **وضعیت**: ⬜ Not Started

### 7.2 Device Listing

- [ ] **FDA Device Listing**
  - [ ] Device listed with FDA
  - [ ] Listing number obtained
  - [ ] Device information submitted
  - **وضعیت**: ⬜ Not Started

### 7.3 Manufacturing Information

- [ ] **Manufacturing Information**
  - [ ] Manufacturing processes documented
  - [ ] Quality control procedures
  - [ ] Facilities information
  - **فایل**: `docs/FDA_510K_Manufacturing_Info.md`
  - **وضعیت**: ⬜ Not Started

---

## ✅ بخش 8: بررسی نهایی

### 8.1 بررسی کیفیت

- [ ] **Internal Review**
  - [ ] Technical review completed
  - [ ] Regulatory review completed
  - [ ] Clinical review completed
  - [ ] Quality review completed
  - **وضعیت**: ⬜ Not Started

- [ ] **Proofreading**
  - [ ] Spelling and grammar checked
  - [ ] Consistency verified
  - [ ] References checked
  - **وضعیت**: ⬜ Not Started

### 8.2 آماده‌سازی ارسال

- [ ] **Document Formatting**
  - [ ] All documents in PDF format
  - [ ] Page numbering consistent
  - [ ] Table of contents updated
  - **وضعیت**: ⬜ Not Started

- [ ] **File Organization**
  - [ ] Files properly named
  - [ ] Directory structure organized
  - [ ] File sizes appropriate
  - **وضعیت**: ⬜ Not Started

### 8.3 Payment

- [ ] **User Fee Payment**
  - [ ] Fee calculated correctly
  - [ ] Payment method confirmed
  - [ ] Payment receipt obtained
  - **وضعیت**: ⬜ Not Started

---

## 📊 خلاصه وضعیت

### پیشرفت کلی

```
[░░░░░░░░░░░░░░░░░░░░] 0% Complete

بخش تکمیل شده: 0/8
فایل‌های ایجاد شده: 2/40+
```

### فایل‌های کلیدی ایجاد شده

- ✅ `docs/FDA_510K_SUBMISSION_GUIDE.md` - راهنمای کامل
- ✅ `docs/FDA_510K_Device_Description.md` - توصیف دستگاه
- ⬜ سایر فایل‌های مستندات

### مراحل بعدی

1. تکمیل فرم‌های FDA
2. آماده‌سازی مستندات نرم‌افزار
3. تکمیل مدیریت ریسک
4. آماده‌سازی شواهد بالینی
5. تکمیل برچسب‌گذاری

---

## 📅 جدول زمانی پیشنهادی

| مرحله | مدت زمان | تاریخ شروع هدف | تاریخ پایان هدف |
|-------|----------|-----------------|------------------|
| آماده‌سازی مستندات | 4-6 ماه | [تاریخ] | [تاریخ] |
| بررسی داخلی | 2-4 هفته | [تاریخ] | [تاریخ] |
| Pre-Submission Meeting | 1-2 ماه | [تاریخ] | [تاریخ] |
| اصلاحات نهایی | 2-4 هفته | [تاریخ] | [تاریخ] |
| ارسال به FDA | 1 هفته | [تاریخ] | [تاریخ] |

---

## 📝 یادداشت‌ها

### یادداشت‌های مهم

- [ ] یادداشت 1
- [ ] یادداشت 2
- [ ] یادداشت 3

### سوالات و مسائل

- [ ] سوال 1: [توضیحات]
- [ ] سوال 2: [توضیحات]
- [ ] سوال 3: [توضیحات]

---

**آخرین بروزرسانی**: [تاریخ]  
**وضعیت**: Draft  
**نسخه**: 1.0

