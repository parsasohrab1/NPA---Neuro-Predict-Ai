...
### 1. **تب "گزارش‌ها و مصورسازی" - ناقص**
**وضعیت**: ⚠️ Partial  
**تاثیر**: بسیار بالا  
**پیچیدگی**: متوسط-بالا

**موجود**:
- ✅ صفحه Reports در Admin Dashboard (تب‌های Clinical، Research، Management)
...
### 2. **تب "ردیابی طولی" - ناقص**
**وضعیت**: ⚠️ Partial  
**تاثیر**: بسیار بالا (برای پیگیری بیماران)  
**پیچیدگی**: بالا

**موجود**:
- ✅ مدل داده و جدول‌های `longitudinal_*`
- ✅ API endpoints برای اپیزود، ویزیت، متریک، timeline، trend و comparison
- ✅ صفحه Longitudinal در داشبورد ادمین (timeline + trend chart + heatmap MRI)
- ✅ محاسبه slope متریک‌های کلیدی و هشدارهای خودکار با امکان تأیید
- ✅ تولید گزارش‌های دوره‌ای (خلاصه JSON + خروجی Excel همراه PDF ساده) و نمایش/دانلود در UI

**کمبودها**:
- ❌ مقایسه تصویری پیشرفته (multi-slice، 3D overlay، alignment دقیق)
- ❌ تحلیل‌های پیشرفته سرعت (baseline شخصی، پیش‌بینی آینده، هشدار ترکیبی)
- ❌ زمان‌بندی و اتوماسیون گزارش‌های دوره‌ای (ماهانه، فصلی، سالانه) و گزارش‌های cohort-level
- ❌ ابزارهای تعاملی (drag/drop تایم‌لاین، لینک عمیق به گزارش‌ها، مقایسه multi-episode)
- ❌ تحلیل‌های جمعی (differences overlay در سطح cohort، volume change heatmap)

**اقدامات مورد نیاز**:
1. ارتقای ابزار مقایسه تصویری (registration، multi-slice، 3D viewer)
2. توسعه الگوریتم‌های پیشرفته سرعت و هشدارهای ترکیبی
3. پیاده‌سازی زمان‌بندی، گزارش‌های cohort-level و خروجی‌های پیشرفته (EHR-ready, multi-format)
4. توسعه UI پیشرفته (نمودارهای چندمحوره، مقایسه چند اپیزود، تعامل drag/zoom)
...
- ✅ آپلود فایل‌های DICOM و اتصال به پرونده بیمار
